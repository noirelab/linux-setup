...
10. **"Turn it off" / "disable"** → `~/.cursor/skills/gstack/bin/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `~/.cursor/skills/gstack/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ~/.cursor/skills/gstack/bin/gstack-developer-profile --read >/dev/null
...
```bash
~/.cursor/skills/gstack/bin/gstack-developer-profile --profile
```
...
```bash
~/.cursor/skills/gstack/bin/gstack-developer-profile --gap
```
...
```bash
~/.cursor/skills/gstack/bin/gstack-question-preference --stats
eval "$(~/.cursor/skills/gstack/bin/gstack-slug 2>/dev/null)"
eval "$(~/.cursor/skills/gstack/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
~/.cursor/skills/gstack/bin/gstack-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
~/.cursor/skills/gstack/bin/gstack-distill-free-text --status
```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ~/.cursor/skills/gstack/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```
