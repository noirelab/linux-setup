...
```bash
~/.cursor/skills/gstack/bin/gstack-gbrain-detect
```
...
```bash
~/.cursor/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.cursor/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.cursor/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.cursor/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.cursor/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.cursor/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.cursor/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.cursor/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.cursor/skills/gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.cursor/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
~/.cursor/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.cursor/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.cursor/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
_HASH=$(~/.cursor/skills/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.cursor/skills/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.cursor/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.cursor/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.cursor/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.cursor/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.cursor/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.cursor/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.cursor/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
