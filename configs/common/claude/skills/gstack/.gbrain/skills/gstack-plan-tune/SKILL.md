...
10. **"Turn it off" / "disable"** → `~/.gbrain/skills/gstack/bin/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `~/.gbrain/skills/gstack/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$(~/.gbrain/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || echo "false")
...
4. If A or B: enable:
   ```bash
   ~/.gbrain/skills/gstack/bin/gstack-config set question_tuning true
   ```
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ~/.gbrain/skills/gstack/bin/gstack-developer-profile --read >/dev/null
...
```bash
~/.gbrain/skills/gstack/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   ~/.gbrain/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.gbrain/skills/gstack/bin/gstack-developer-profile --gap
```
...
```bash
~/.gbrain/skills/gstack/bin/gstack-question-preference --stats
eval "$(~/.gbrain/skills/gstack/bin/gstack-slug 2>/dev/null)"
eval "$(~/.gbrain/skills/gstack/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
~/.gbrain/skills/gstack/bin/gstack-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
~/.gbrain/skills/gstack/bin/gstack-distill-free-text --status
```
...
1. Show the proposals:
   ```bash
   ~/.gbrain/skills/gstack/bin/gstack-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ~/.gbrain/skills/gstack/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   ~/.gbrain/skills/gstack/bin/gstack-distill-apply --proposal N
   ```
...
1. Run distill:
   ```bash
   ~/.gbrain/skills/gstack/bin/gstack-distill-free-text
   ```
...
```bash
~/.gbrain/skills/gstack/bin/gstack-distill-free-text --background
```
