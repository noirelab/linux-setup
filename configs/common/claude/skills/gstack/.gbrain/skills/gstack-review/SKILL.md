...
```bash
~/.gbrain/skills/gstack/bin/gstack-learnings-log '{
  "type": "pitfall",
...
Read `.gbrain/skills/review/checklist.md`.
...
Read `.gbrain/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
```bash
~/.gbrain/skills/gstack/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
