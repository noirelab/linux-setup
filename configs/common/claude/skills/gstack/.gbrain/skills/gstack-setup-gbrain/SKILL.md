...
```bash
~/.gbrain/skills/gstack/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.gbrain/skills/gstack/bin/gstack-gbrain-detect`
...
```bash
~/.gbrain/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.gbrain/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.gbrain/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.gbrain/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.gbrain/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.gbrain/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.gbrain/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.gbrain/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.gbrain/skills/gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.gbrain/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.gbrain/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.gbrain/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.gbrain/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.gbrain/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.gbrain/skills/gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.gbrain/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.gbrain/skills/gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$(~/.gbrain/skills/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.gbrain/skills/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.gbrain/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.gbrain/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.gbrain/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.gbrain/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.gbrain/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.gbrain/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.gbrain/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
