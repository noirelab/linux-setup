...
11. **"Turn it on" / "enable"** → `~/.hermes/skills/gstack/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
4. Write:
   ```bash
   ~/.hermes/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.hermes/skills/gstack/bin/gstack-question-preference --stats
eval "$(~/.hermes/skills/gstack/bin/gstack-slug 2>/dev/null)"
eval "$(~/.hermes/skills/gstack/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
