...
**If prior retros exist:** Load the most recent one using the read_file tool. Calculate deltas for key metrics and include a **Trends vs Last Retro** section:
```
                    Last        Now         Delta
...
Use the patch tool to save the JSON file with this schema:
```json
{
  "date": "2026-03-08",
  "window": "7d",
...
```bash
DISCOVER_BIN=""
[ -x ~/.hermes/skills/gstack/bin/gstack-global-discover ] && DISCOVER_BIN=~/.hermes/skills/gstack/bin/gstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .hermes/skills/gstack/bin/gstack-global-discover ] && DISCOVER_BIN=.hermes/skills/gstack/bin/gstack-global-discover
...
If a matching prior retro exists, load it with the read_file tool. Show a **Trends vs Last Global Retro** table with deltas for key metrics: total commits, LOC, sessions, streak, context switches/day.
...
Use the patch tool to save JSON to `~/.gstack/retros/global-${today}-${next}.json`:
