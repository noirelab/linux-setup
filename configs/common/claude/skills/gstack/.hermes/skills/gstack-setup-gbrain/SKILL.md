...
```bash
~/.hermes/skills/gstack/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.hermes/skills/gstack/bin/gstack-gbrain-detect`
...
```bash
~/.hermes/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.hermes/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.hermes/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.hermes/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.hermes/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.hermes/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.hermes/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.hermes/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.hermes/skills/gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.hermes/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.hermes/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.hermes/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
...
try:
    ...
    pass
")
~/.hermes/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.hermes/skills/gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.hermes/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.hermes/skills/gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
~/.hermes/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.hermes/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.hermes/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.hermes/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.hermes/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.hermes/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
