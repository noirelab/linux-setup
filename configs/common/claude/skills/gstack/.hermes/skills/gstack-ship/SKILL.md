...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.hermes/skills/review/TODOS-format.md`).
...
`~/.hermes/skills/gstack/bin/gstack-detach` instead — it survives in its own
...
```bash
~/.hermes/skills/gstack/bin/gstack-detach --label ship-evals --lock gstack-evals --timeout 5400 -- <project eval command>
```
...
```bash
eval "$(~/.hermes/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
...
1. Read `.hermes/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.hermes/skills/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.hermes/skills/gstack/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `.hermes/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
1. **Classify state** — pure reader, never writes:
   ```bash
   bun run ~/.hermes/skills/gstack/bin/gstack-version-bump classify --base <base>
   ```
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run ~/.hermes/skills/gstack/bin/gstack-next-version --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run ~/.hermes/skills/gstack/bin/gstack-version-bump write --version "$NEW_VERSION"
   ```
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
   ~/.hermes/skills/gstack/bin/gstack-decision-log '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
Read `.hermes/skills/review/TODOS-format.md` for the canonical format reference.
...
```bash
_REDACT_PREPUSH=$(~/.hermes/skills/gstack/bin/gstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   ~/.hermes/skills/gstack/bin/gstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `~/.hermes/skills/gstack/bin/gstack-config set redact_prepush_hook true`
   then `~/.hermes/skills/gstack/bin/gstack-redact install-prepush-hook`.
   If B: run `~/.hermes/skills/gstack/bin/gstack-config set redact_prepush_hook false`.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.hermes/skills/gstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
2. Compute the corrected title: `NEW_TITLE=$(~/.hermes/skills/gstack/bin/gstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
<Auto-detect: look for /spec archives matching this branch via:
  eval "$(~/.hermes/skills/gstack/bin/gstack-paths)"
  eval "$(~/.hermes/skills/gstack/bin/gstack-slug)"
...
<PR body from above>
PR_BODY_EOF
~/.hermes/skills/gstack/bin/gstack-redact --from-file "$PR_BODY_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json
case $? in
...
printf '%s' "v$NEW_VERSION <type>: <summary>" | ~/.hermes/skills/gstack/bin/gstack-redact --repo-visibility "$REDACT_VIS" --json
```
