...
```bash
~/.kiro/skills/gstack/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.kiro/skills/gstack/bin/gstack-gbrain-detect`
...
```bash
~/.kiro/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.kiro/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.kiro/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.kiro/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.kiro/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.kiro/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.kiro/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.kiro/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.kiro/skills/gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.kiro/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.kiro/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.kiro/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.kiro/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.kiro/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.kiro/skills/gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.kiro/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.kiro/skills/gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$(~/.kiro/skills/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.kiro/skills/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.kiro/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.kiro/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.kiro/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.kiro/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.kiro/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.kiro/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.kiro/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
