...
```bash
~/.openclaw/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
```bash
cat ~/.openclaw/skills/gstack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $(~/.openclaw/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
