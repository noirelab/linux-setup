...
```bash
~/.openclaw/skills/gstack/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.openclaw/skills/gstack/bin/gstack-gbrain-detect`
...
```bash
~/.openclaw/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.openclaw/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.openclaw/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.openclaw/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.openclaw/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.openclaw/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.openclaw/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.openclaw/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.openclaw/skills/gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.openclaw/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.openclaw/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.openclaw/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.openclaw/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.openclaw/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.openclaw/skills/gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.openclaw/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.openclaw/skills/gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$(~/.openclaw/skills/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.openclaw/skills/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.openclaw/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.openclaw/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.openclaw/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.openclaw/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.openclaw/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.openclaw/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.openclaw/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
