...
$GSTACK_BIN/gstack-timeline-log '{"skill":"document-generate","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  ~/.slate/skills/gstack/bin/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
