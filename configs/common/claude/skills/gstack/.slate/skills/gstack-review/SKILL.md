...
```bash
~/.slate/skills/gstack/bin/gstack-learnings-log '{
  "type": "pitfall",
...
Read `.slate/skills/review/checklist.md`.
...
Read `.slate/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
```bash
~/.slate/skills/gstack/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
