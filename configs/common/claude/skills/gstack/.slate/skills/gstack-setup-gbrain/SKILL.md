...
$GSTACK_BIN/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.slate/skills/gstack/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.slate/skills/gstack/bin/gstack-gbrain-detect`
...
```bash
~/.slate/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.slate/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.slate/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.slate/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.slate/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.slate/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.slate/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.slate/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.slate/skills/gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.slate/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.slate/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.slate/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.slate/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.slate/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.slate/skills/gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.slate/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.slate/skills/gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$(~/.slate/skills/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.slate/skills/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.slate/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.slate/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.slate/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.slate/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.slate/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.slate/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.slate/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
