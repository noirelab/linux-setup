...
GSTACK_ROOT="$HOME/.slate/skills/gstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.slate/skills/gstack" ] && GSTACK_ROOT="$_ROOT/.slate/skills/gstack"
GSTACK_BIN="$GSTACK_ROOT/bin"
GSTACK_BROWSE="$GSTACK_ROOT/browse/dist"
GSTACK_DESIGN="$GSTACK_ROOT/design/dist"
_UPD=$($GSTACK_BIN/gstack-update-check 2>/dev/null || .slate/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
...
_VENDORED="no"
if [ -d ".slate/skills/gstack" ] && [ ! -L ".slate/skills/gstack" ]; then
  if [ -f ".slate/skills/gstack/VERSION" ] || [ -d ".slate/skills/gstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
> This project has gstack vendored in `.slate/skills/gstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .slate/skills/gstack/`
2. Run `echo '.slate/skills/gstack/' >> .gitignore`
3. Run `$GSTACK_BIN/gstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate gstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd ~/.slate/skills/gstack && ./setup --team`"
...
~/.slate/skills/gstack/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"SKILL_NAME","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -x ~/.slate/skills/gstack/bin/gstack-telemetry-log ]; then
  ~/.slate/skills/gstack/bin/gstack-telemetry-log \
...
```bash
~/.slate/skills/gstack/bin/gstack-review-read
```
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.slate/skills/review/TODOS-format.md`).
...
`~/.slate/skills/gstack/bin/gstack-detach` instead — it survives in its own
...
```bash
~/.slate/skills/gstack/bin/gstack-detach --label ship-evals --lock gstack-evals --timeout 5400 -- <project eval command>
```
...
```bash
eval "$(~/.slate/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
...
1. Read `.slate/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.slate/skills/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.slate/skills/gstack/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `.slate/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
_TEL=$(~/.slate/skills/gstack/bin/gstack-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$(~/.slate/skills/gstack/bin/gstack-config get codex_reviews 2>/dev/null || echo enabled)
source ~/.slate/skills/gstack/bin/gstack-codex-probe 2>/dev/null || true
...
```bash
~/.slate/skills/gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.slate/skills/gstack/bin/gstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
1. **Classify state** — pure reader, never writes:
   ```bash
   bun run ~/.slate/skills/gstack/bin/gstack-version-bump classify --base <base>
   ```
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run ~/.slate/skills/gstack/bin/gstack-next-version --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run ~/.slate/skills/gstack/bin/gstack-version-bump write --version "$NEW_VERSION"
   ```
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
   ~/.slate/skills/gstack/bin/gstack-decision-log '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
Read `.slate/skills/review/TODOS-format.md` for the canonical format reference.
...
```bash
_REDACT_PREPUSH=$(~/.slate/skills/gstack/bin/gstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   ~/.slate/skills/gstack/bin/gstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `~/.slate/skills/gstack/bin/gstack-config set redact_prepush_hook true`
   then `~/.slate/skills/gstack/bin/gstack-redact install-prepush-hook`.
   If B: run `~/.slate/skills/gstack/bin/gstack-config set redact_prepush_hook false`.
...
2. Compute the corrected title: `NEW_TITLE=$(~/.slate/skills/gstack/bin/gstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
<Auto-detect: look for /spec archives matching this branch via:
  eval "$(~/.slate/skills/gstack/bin/gstack-paths)"
  eval "$(~/.slate/skills/gstack/bin/gstack-slug)"
...
```bash
REDACT_VIS=$(~/.slate/skills/gstack/bin/gstack-config get redact_repo_visibility 2>/dev/null)
...
<PR body from above>
PR_BODY_EOF
~/.slate/skills/gstack/bin/gstack-redact --from-file "$PR_BODY_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json
case $? in
...
printf '%s' "v$NEW_VERSION <type>: <summary>" | ~/.slate/skills/gstack/bin/gstack-redact --repo-visibility "$REDACT_VIS" --json
```
...
_QT=$(~/.slate/skills/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
