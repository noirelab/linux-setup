...
instead of WebSearch for SERP data. Run `/home/noirelab/.config/opencode/skills/seo/bin/claude-seo run dataforseo_costs.py check serp_organic_live_advanced --count N`
...
- All URLs fetched via `/home/noirelab/.config/opencode/skills/seo/bin/claude-seo run render_page.py <url> --mode auto` (SPA-aware SSRF protection via `url_safety`)
